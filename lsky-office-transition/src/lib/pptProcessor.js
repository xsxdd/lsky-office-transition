...
class PowerPointProcessor {
   ...
   */
  static async extract(pptxPath, extractPath) {
    try {
      // 确保目录存在
      ...
      console.log(`PPT已解压到: ${extractPath}`);
      return extractPath;
    } catch (error) {
      console.error('解压PowerPoint文档时出错:', error);
      throw error;
    }
  }
  ...
  static async create(folderPath, outputPath) {
    try {
      ...
      console.log(`PowerPoint文档已创建: ${outputPath}`);
      return outputPath;
    } catch (error) {
      console.error('创建PowerPoint文档时出错:', error);
      throw error;
    }
  }
